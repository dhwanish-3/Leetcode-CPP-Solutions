...
void dfs(vector<int>& size, vector<vector<int>>& adjList, int u, vector<int>& dp) {
    dp[u] = size[u];
    int len = adjList[u].size();
    for (int i = 0; i < len; i++) {
        dfs(size, adjList, adjList[u][i], dp);
        dp[u] += dp[adjList[u][i]];
    }
}
...
int mostBalancedPartition(vector<int> parent, vector<int> size) {
    ...
    vector<int> dp(n, 0);
    dfs(size, adjList, 0, dp);
    int sol = INT_MAX;
    for (int i = 0; i < n; i++) {
        sol = min(sol, abs(totalSize - 2 * dp[i]));
    }
    return sol;
}
